...
public class Game extends Canvas implements Runnable {
   ...
   */
  public enum STATE {
    Menu, Help, Help2, Help3, Game, GameOver, GameWon, Upgrade, Boss, Pause, PauseH1, PauseH2, PauseH3, PauseShop,
    Leaderboard, GameWonEasy, GameEasy
  };
  ...
  public Game() throws MalformedURLException {
    ...
    mouseListener = new MouseListener(this, this.handler, this.hud, this.spawnerE, this.spawner, this.spawner2,
        this.upgradeScreen, this.player, this.upgrades, pause);
    this.addKeyListener(
        new KeyInput(this.pause, this.handler, this, this.hud, this.player, this.spawner, this.upgrades));
  ...
  // This sets the game stats when the user goes into the wave game mode, takes
  // stuff from the saved game
  ...
  private void tick() throws IOException, JSONException {
    if (gameState == STATE.Pause || gameState == STATE.PauseShop || gameState == STATE.PauseH1
        || gameState == STATE.PauseH2 || gameState == STATE.PauseH3 || gameState == STATE.Leaderboard) {
      ...
      if (gameState == STATE.Game || gameState == STATE.Boss && Spawn1to10.LEVEL_SET != 3) {// game is running
        ...
        if (SpawnEasy.LEVEL_SET == 1) {
          ...
          || gameState == STATE.Help3) {// user is on menu, update the menu items
        menu.tick();
   ...
   * screen
   * 
   * @param var x or y location of entity
   * @param min minimum value still on the screen
   * @param max maximum value still on the screen
